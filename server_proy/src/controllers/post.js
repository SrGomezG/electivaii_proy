...
const createPost = async (req, res) => {
    try {
        ...
        const post = new Post({
            title, miniature, content, category, path,
        });
...
const getPostsBySearch = async (req, res) => {
    try {
        ...
        if (posts.length === 0) {
            return res.status(404).send({
                message: "No se han encontrado posts para el término de búsqueda especificado."
            });
        }
        res.status(200).send({
            posts,
        ...
        console.error(err);
        res.status(500).send({
            message: "Error en el servidor al obtener los posts por término de búsqueda.",
        });
    }
};
